...
const Checkout = () => {
  ...
  const handlePaymentSuccess = async (paymentResult: any) => {
    console.log("Payment success data:", paymentResult);
    ...
    try {
      ...
      const addOnDetails = {
        ...
        backupCopies: { selected: cloudBackup > 0, cost: 0 }
      };
      ...
      const orderData = {
        orderId: orderId,
        customerInfo: { ...validatedFormData, fullName: `${validatedFormData.firstName} ${validatedFormData.lastName}` },
        orderDetails: {
          package: packageType,
        ...
        paymentMethod: cardDetails ? `Credit Card (${cardDetails.brand} ending in ${cardDetails.last4})` : 'Credit Card',
        timestamp: new Date().toISOString()
      };
      ...
      if (usbDrives > 0) params.append('usbDrives', usbDrives.toString());
      if (cloudBackup > 0) params.append('cloudBackup', cloudBackup.toString());
